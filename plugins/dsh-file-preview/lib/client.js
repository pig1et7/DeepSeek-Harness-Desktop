...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var panelStyle = {
      ...
      background: "var(--dsw-specific-panel, #0f1526)",
      borderRight: "1px solid var(--dsw-alias-border-l2-darkmode-thin, #232c44)",
      boxShadow: "var(--dsw-shadow-lv3, 0 8px 40px rgba(0,0,0,.4))",
      zIndex: 999, display: "flex", flexDirection: "column",
      fontFamily: "var(--dsw-font-family, Segoe UI, Microsoft YaHei, sans-serif)",
      fontSize: 13, color: "var(--dsw-alias-label-primary, #dbe4f5)",
    };
    ...
    function PreView(props) {
      ...
      if (kind === "text") {
        return h("div", { style: { display: "flex", flexDirection: "column", flex: "auto", minHeight: 0 }, children: [
          h("pre", { key: "p", style: { margin: 0, padding: 10, fontSize: 12, lineHeight: 1.55, fontFamily: "var(--ds-font-family-code, Consolas, monospace)", color: "var(--dsw-alias-label-primary, #dbe4f5)", whiteSpace: "pre", overflow: "auto" }, children: content }),
          truncated ? h("div", { key: "t", style: { padding: "6px 10px", flex: "none", color: "var(--dsw-alias-label-tertiary, #5d6f92)", fontSize: 12 }, children: "… 内容过长已截断" }) : null,
        ] });
      }
      if (kind === "image") {
        return h("div", { style: { padding: 12, overflow: "auto", flex: "auto", textAlign: "center" }, children: [
          ...
          h("div", { key: "m", style: { marginTop: 8, color: "var(--dsw-alias-label-tertiary, #5d6f92)", fontSize: 12 }, children: (width && height ? width + " × " + height + " px" : "") }),
        ] });
      }
      return h("div", { style: { padding: 14, color: "var(--dsw-alias-label-secondary, #b9c8e8)" }, children: hint || "无法预览" });
    }

    function DiffView(props) {
      var diffs = props.diffs;
      if (!diffs || diffs.length === 0) {
        return h("div", { style: { padding: 14, color: "var(--dsw-alias-label-tertiary, #5d6f92)" }, children: "无 diff 数据" });
      }
      var blocks = [];
      diffs.forEach(function (d, idx) {
        ...
        if (d.title) {
          blocks.push(h("div", { key: "h" + idx, style: { padding: "4px 10px", background: "var(--dsw-alias-interactive-bg-hover, #141b2e)", fontFamily: "var(--ds-font-family-code, Consolas, monospace)", fontSize: 12 }, children: d.title }));
        }
        ...
        for (var i = 0; i < max; i++) {
          ...
          var style = {
            padding: "0 10px", whiteSpace: "pre", fontFamily: "var(--ds-font-family-code, Consolas, monospace)",
            fontSize: 12, lineHeight: 1.5,
            background: isAdd ? "rgba(46,160,67,.15)" : isDel ? "rgba(248,81,73,.15)" : "transparent",
            color: isAdd ? "#7ad9a0" : isDel ? "#ff9b92" : "var(--dsw-alias-label-secondary, #b9c8e8)",
          };
    ...
    function PreviewPanel(props) {
      ...
      var listRows = files.map(function (f) {
        ...
        var badge = null;
        if (f.plus + f.minus > 0) {
          badge = h("span", { style: { flex: "none", borderRadius: 6, padding: "0 5px", fontSize: 11, fontFamily: "var(--ds-font-family-code, Consolas, monospace)", background: "rgba(46,160,67,.12)", color: "#7ad9a0" }, children: (f.plus > 0 ? "+" + f.plus + " " : "") + (f.minus > 0 ? "-" + f.minus : "") });
        }
        return h("div", {
          key: f.path,
          style: Object.assign({ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", padding: "7px 12px", borderRadius: 8, minWidth: 0 }, rowSelected ? { background: "var(--dsw-alias-interactive-bg-hover, #141b2e)" } : {}),
          ...
          badge,
          h("span", { key: "t", style: { flex: "none", color: "var(--dsw-alias-label-tertiary, #5d6f92)", fontSize: 11 }, children: fmtTime(f.updatedAt) }),
        ]);
      });

      var headerStyle = {
        display: "flex", alignItems: "center", gap: 8, padding: "10px 12px",
        borderBottom: "1px solid var(--dsw-alias-border-l2-darkmode-thin, #232c44)", flex: "none",
      };
      var smallBtn = {
        display: "inline-flex", alignItems: "center", justifyContent: "center", width: 26, height: 26,
        borderRadius: 8, border: 0, background: "transparent", color: "var(--dsw-alias-label-secondary, #8fa1c4)",
      ...
      var tabStyle = function (active) {
        return {
          ...
          background: active ? "var(--dsw-alias-interactive-bg-hover, #141b2e)" : "transparent",
          color: active ? "var(--dsw-alias-label-primary, #dbe4f5)" : "var(--dsw-alias-label-tertiary, #5d6f92)",
        };
      };
      ...
      return h("div", { style: panelStyle, "data-file-preview-panel": "", role: "dialog" }, [
        ...
        h("div", { key: "l", style: { flex: "none", maxHeight: "38vh", overflow: "auto", borderBottom: "1px solid var(--dsw-alias-border-l2-darkmode-thin, #232c44)", padding: 6 }, children: [
          files.length === 0
            ? h("div", { style: { padding: 14, color: "var(--dsw-alias-label-tertiary, #5d6f92)", textAlign: "center" }, children: "暂无文件记录 —— Agent 写/改文件后会出现在这里" })
            : listRows,
        ] }),
        (selected
        ...
        h("div", { key: "b", style: { flex: "auto", minHeight: 0, display: "flex", flexDirection: "column" }, children: [
          loading
            ? h("div", { style: { padding: 14, color: "var(--dsw-alias-label-tertiary, #5d6f92)" }, children: "加载中…" })
            : error
              ? h("div", { style: { padding: 14, color: "var(--dsw-alias-state-error-primary, #ff7a7a)" }, children: error })
              : !selected
                ? h("div", { style: { padding: 14, color: "var(--dsw-alias-label-tertiary, #5d6f92)" }, children: "点击上方文件查看内容与 diff" })
                : tab === "diff" && diff && diff.ok
                  ...
                  : (content && content.ok)
                    ...
                    : h("div", { style: { padding: 14, color: "var(--dsw-alias-state-error-primary, #ff7a7a)" }, children: (content && content.message) || "加载失败" }),
        ] }),
      ]);
    }
    ...
    function PanelToggle() {
      ...
      var btnStyle = {
        ...
        background: "transparent", color: "var(--dsw-alias-label-secondary, #8fa1c4)",
